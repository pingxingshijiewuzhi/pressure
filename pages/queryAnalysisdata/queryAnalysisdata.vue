<template>
	<view class="content">
		...
		<view class="head flexs2">
			...
			<view class="title">
				<view class="text-white datadate">
					{{pickerDate}}
					<view>{{langtype=='CH'?langch.datasearch:langen.datasearch}}</view>
				</view>
			</view>
		...
		<!-- 日期选择完成切换显示页面 -->
		<view v-if='validdatalist.length == 0' class='datacontent'>
		...
		<view v-if='validdatalist.length != 0' class='datacontent'>
			...
			<view class="bigbox" v-show="Inv == 0">
				...
				<scroll-view scroll-y="true" class="biglist" @scrolltolower='scrolltolower'>
					<!-- @scrolltoupper='scrolltoupper'下拉刷新关闭 -->
					<view class="list_one flexs2 " v-for="(item,index) in datalist" :key="index">
						...
						<!-- 把item.l 改成 item.l ！= ' ' 判断条件一样效果 -->
							...
							<checkbox-group @change="checkboxChange(index)" v-if="item.l!= ' '">
								<checkbox class='round blue' :class="{checked:item.checked}" value="C"></checkbox>
							</checkbox-group>
						</view>
						<view class="list2 flexs">
			...
			<view class="bottombox text-center flexs3">
				<view class="submit addbackcolor" @click='lookreport'>{{langtype=='CH'?langch.chakanbaogao:langen.chakanbaogao}}</view>
				<!-- <view class="submit addbackcolor">{{langtype=='CH'?langch.chakanstaticbaogao:langen.chakanstaticbaogao}}</view> -->
				<view class="submit addbackcolor" @click='deleteclick'>{{langtype=='CH'?langch.shanchu:langen.shanchu}}</view>
		...
		<!-- 模态框 -->
		<delete-hint :isShow='isShow' @cancleclick='cancleclick' @tureclick='tureclick' :title='title' @closemod='closemod'></delete-hint>
		<!-- 上拉加载显示 -->
		<view class="uni-loadmore" v-if="showLoadMore">{{langtype=='CH'?langch.loadMoreText:langen.loadMoreText}}</view>
	</view>
</template>
<script>
	...
	export default {
		...
		data() {
			return {
				...
				langen: {
					...
					chakanstaticbaogao: 'Static View report',
		...
		onShow() {
			...
			for (var i = 0; i < this.validdatalist.length; i++) {
				this.validdatalist[i].checked = false
			}
		...
		methods: {
			...
			checkboxChange(index) {
				...
				// 每次单击确认是否存在选中
				// var newArr = this.datalist.filter(item => {
			...
			checkboxChange1(index) {
				...
				// 每次单击确认是否存在选中
				// var newArr = this.validdatalist.filter(item => {
				// 	return item.checked == true
				// })
				// if (newArr.length != 0) {
				// 	this.deletestatus = true
			...
			checkboxChange2(index) {
				...
				// 每次单击确认是否存在选中
				// var newArr = this.garbagedatalist.filter(item => {
				// 	return item.checked == true
				// })
				// if (newArr.length != 0) {
				// 	this.deletestatus = true
			...
			checkboxChange3(index) {
				...
				// 每次单击确认是否存在选中
				// var newArr = this.commonlist.filter(item => {
				// 	return item.checked == true
				// })
				// if (newArr.length != 0) {
				// 	this.deletestatus = true
			...
			lookreport() {
				...
				if (this.Inv == 0) {
					...
					// console.log(checkedid,'数据没传到下一个页面?')
					// 把当前选中id带到折线图页面
					uni.navigateTo({
						url: '../bestlinechart/bestlinechart?checkedid=' + checkedid + '&pickerDate=' + this.pickerDate + '&Inv=0'
					})
				}
				if (this.Inv == 1) {
					this.validdatalist.forEach(item => {
					...
					// 把当前选中id带到折线图页面
					uni.navigateTo({
						url: '../linechart/linechart?checkedid=' + checkedid + '&pickerDate=' + this.pickerDate + '&Inv=1'
					})
				}
			...
			async mywriteBLECharacteristicValue() {	
				...
				var pickertime = this.pickerDate
				...
				printerJobs
					...
					.print(printerUtil.inline('DATE', pickertime))
					.print(printerUtil.fillLine())
					.setSize(2, 2)
					.print('TIME    L    R')
					.println();
				...
				printerJobs
					.print("")
					.print("")
				let buffer = await printerJobs.buffer();
