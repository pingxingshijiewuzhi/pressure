<template>
	<view class="content">
		<view class="head flexs2">
			...
			<view class="title">
				<view class="text-white">月曲线报告查询</view>
			</view>
		...
		<view class='datacontent'>
			<view class='choosetime'>
				<block v-for='(item,index) in timeArray' :key='index'>
					<view class='choosetime_item' :class="{'addchoosetime_item_color':currentIndex == index}" @click='choose_item(index)'>
						{{item}}
					</view>
				</block>
			</view>
		...
		methods: {
			...
			_gettime() {
				...
				return new Promise((resolve, reject) => {
					that.$common.requests('/api/record/getTimeList').then(res => {
						if (res.data.code == 1) {
			...
			_searchData() {
				var that = this
				this._gettime().then(res => {
					let obj = {
						year: this.year,
						month: this.month,
						hour: that.timeArray[that.currentIndex]
					}
					that.$common.requests('/api/record/getmonthinfo_zhexian', obj).then(res => {
						// console.log(res.data.data.lr)
						that.option.dataset.source = res.data.data.lr
						// console.log(res.data.data)
						that.echartdata = res.data.data
					})
				})
