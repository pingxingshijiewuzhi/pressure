...
class Bluetooth {
	...
	stopBluetoothDevicesDiscovery() {
		...
		return new Promise((resolve, reject) => {
			uni.stopBluetoothDevicesDiscovery({
				...
				fail: e => {
					uni.hideLoading();
					console.log(`停止搜索蓝牙设备失败` + JSON.stringify(err))
	...
	createBLEConnection() {
		...
		return new Promise((resolve, reject) => {
			uni.createBLEConnection({
				...
				fail: err => {
					uni.hideLoading();
					console.log(`停止搜索蓝牙设备失败222` + JSON.stringify(err))
					// 重复连接不提示,否则报错提示
					if(err.errCode != -1){
						self.showToast(`停止搜索蓝牙设备失败` + JSON.stringify(err));
					}
	...
	notifyBLECharacteristicValue() {
		...
		console.log(deviceId,serviceId,characteristicId,99999)
	...
	reconnect() {
		(async () => {
			try {
					...
					// 不需要监听变化,我只要queryAnalysisdata打印
					// let result4 = await this.notifyBLECharacteristicValue();
				
				
				// this.writeId = uni.getStorageSync("writeId");
				// this.notifyId = uni.getStorageSync("notifyId");
			} catch (err) {
				// 已经连接报错,重复连接,充分连接我也接着获取他的设备服务和设备服务特征值
				if(err.errCode == -1){
					let result2 = await this.getBLEDeviceServices();
					// console.log("getBLEDeviceServices: " + JSON.stringify(result2));
